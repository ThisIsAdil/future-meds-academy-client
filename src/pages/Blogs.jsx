    ...
    <div className='space-y-8 py-16'>
      ...
      <div className='flex flex-wrap justify-between items-center max-w-7xl mx-auto px-4 gap-4'>
        <div className='flex flex-wrap gap-2'>
          {
            ["All", "Study Abroad & Visa", "Universities In Italy", "Scholarships & Finance", "Success Stories"].map((e, index) => (
              <span key={index} className={`px-6 py-2 rounded-sm cursor-pointer ${activeTab == index ? "bg-(--accent-dark) text-(--accent-light)" : ""}`} onClick={() => setActiveTab(index)}>{e}</span>
            ))
          }
        </div>
          ...
          blogsContent.sort((a, b) => {
            ...
            <Link to={`/blogs/${blog._id}`} key={index} className='flex flex-col p-4 gap-2 bg-(--accent-light) rounded-md cursor-pointer'>
              <div className='relative'>
                <img src={blog.thumbnail} alt={blog.title} className='w-full aspect-video object-cover rounded-md' />
